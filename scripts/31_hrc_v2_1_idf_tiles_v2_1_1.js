// =====================================================================
// 31_hrc_v2_1_idf_tiles_v2_1_1.js — v2.1.1 higher-fidelity (patch)
// Île-de-France 500-metre HRC pipeline
//
// SUPERSEDES script 31_hrc_v2_1_idf_tiles.js (v2.1.0). The pipeline is
// identical except for two changes documented in v2.1.1:
//
//   1. LST DAY/NIGHT EQUAL-WEIGHT AGGREGATION (the fix)
//      Pre-fix:  dayLwUp.merge(nightLwUp).mean()
//      Post-fix: dayLwUp.mean()  +  nightLwUp.mean()  → / 2
//
//      The pre-fix pattern over-weighted day observations because the QC
//      mask passes more day scenes than night scenes. Day LST > night LST,
//      so the day-weighted pooled mean inflated annual mean LW_up by
//      ~10-15 W/m² depending on the day/night QC pass-rate ratio. That
//      depressed Rn, inflated EF, inflated HRC by ~0.3–1.1 units. See
//      methodology paper §4.7 and process guide §6.5.
//
//   2. SINGLE-PIXEL DIAGNOSTIC AT FR-FON
//      The script now prints the HRC value at the FR-Fon tower pixel
//      directly to the console, so the acceptance-gate value can be
//      read without a separate database query.
//
// v2.1.1 acceptance gate at FR-Fon:
//   matched-methodology reference = 6.18, tolerance ±0.5
//   v2.1.1 measured (production)  = 5.70, gap −0.48 → PASS
// =====================================================================
...
print('Annual latent heat range (IDF — expect mean ~2.0e9 J/m²/yr):',
...
// ── 3. Net longwave: LW_down − LW_up (v2.1.1 equal-weight aggregation)
...
// v2.1.1 FIX: compute day and night annual means independently, then
// average with equal weight. Removes day/night QC pass-rate sample-balance
// bias documented in methodology paper §4.7 and process guide §6.5.
...
print('HRC range (IDF v2.1.1 — expect mean ~5.92, was ~6.7 pre-fix):',
  hrc.reduceRegion({
    reducer: ee.Reducer.minMax().combine(ee.Reducer.mean(), '', true),
    geometry: BBOX, scale: 500, maxPixels: 1e9
  })
...
// ── 5a. ACCEPTANCE GATE DIAGNOSTIC — FR-Fon tower pixel ──────────────
// Prints the satellite HRC at the FR-Fon tower pixel directly to console.
// This is the v2.1.1 acceptance gate value. Expected: 5.70 ± 0.1.
var FR_FON = ee.Geometry.Point([2.780, 48.476]);
print(
  'ACCEPTANCE GATE: HRC at FR-Fon pixel ' +
  '(matched-methodology reference = 6.18 ± 0.5; expected ~5.70):',
  hrc.addBands(hrc_raw_ratio).reduceRegion({
...
print('Sample point count (IDF — expect ~15,994):', samplePoints.size());
...
Map.addLayer(FR_FON, { color: 'red' },
  'FR-Fon tower (acceptance gate target 6.18 ± 0.5)');

// ── 8. Export ────────────────────────────────────────────────────────
Export.table.toDrive({
  collection:     samplePoints,
  description:    'hrc_v2_1_1_idf_tiles',
  folder:         'EarthHRC',
  fileNamePrefix: 'hrc_v2_1_1_idf_tiles',
  fileFormat:     'CSV',
  selectors: [
