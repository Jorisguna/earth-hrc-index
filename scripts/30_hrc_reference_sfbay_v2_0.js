...
// Earth HRC Index — Intact Site Reference — SF Bay — v2.0
//
// Computes hrc_reference per RESOLVE ecoregion as the p90 HRC
// of IUCN I–IV protected area centroids within the SF Bay region.
//
...
//
// Note: If IUCN I–IV yields < 10 centroids per ecoregion,
//       consider expanding to I–VI (as done for LA in script 29).
//       Check pa_centroid_count in the export before importing.
//
// Datasets:
...
print('HRC range (SF Bay — expect mean ~4.5–6.0):',
...
// ── Step 2: Load WDPA — IUCN I–VI (SF Bay deviation) ────────
// SF Bay has very sparse IUCN I–IV coverage (2–3 centroids per ecoregion).
// Expanding to I–VI gives enough samples for a viable benchmark.
// reference_filter field records 'IUCN_I-VI' in output.
var wdpa = ee.FeatureCollection('WCMC/WDPA/current/polygons')
  .filter(ee.Filter.inList('IUCN_CAT', ['Ia', 'Ib', 'II', 'III', 'IV', 'V', 'VI']))
  .filterBounds(region);

print('WDPA I–VI PA count in SF Bay region:', wdpa.size());
...
print('Ecoregion count in SF Bay region:', resolve.size());
...
Map.addLayer(wdpa, { color: '2E7D32' }, 'WDPA I–VI (SF Bay)');

// ── Step 7: Export ───────────────────────────────────────────
Export.table.toDrive({
  collection:     results,
  description:    'hrc_reference_sfbay_v2_0',
  folder:         'EarthHRC',
  fileNamePrefix: 'hrc_reference_sfbay_v2_0',
  fileFormat:     'CSV',
