// =====================================================================
// 32_hrc_v2_1_tapajos_tiles_v2_1_1.js — v2.1.1 higher-fidelity (patch)
// Tapajós 500-metre HRC pipeline
//
// SUPERSEDES script 32_hrc_v2_1_tapajos_tiles.js (v2.1.0). Same fix and
// diagnostic-print additions as 31_hrc_v2_1_idf_tiles_v2_1_1.js — see
// that script's header for full commentary.
//
// v2.1.1 K67 calibration measurement (not a pass-fail gate):
//   matched-methodology reference = 7.65, tolerance ±2.0 (documented
//     MOD11A1 cold-bias range in dense humid tropical evergreen forest)
//   v2.1.1 measured (production)  = 5.62, gap −2.03
//   Verdict: WITHIN_DOCUMENTED_BIOME_LIMITATION
//   Root cause: MOD11A1 reads 6.5 K below tower radiometric temperature.
//   Fix is upstream (ECOSTRESS at 70 m) — out of scope for v2.1.1.
//   See methodology paper §7.8 and §10.8.
// =====================================================================
...
// v2.1.1 FIX: equal-weight day/night aggregation. See script 31_v2_1_1
...
print('HRC range (Tapajós v2.1.1 — expect mean ~5.71, was ~6.0 pre-fix):',
  hrc.reduceRegion({
    reducer: ee.Reducer.minMax().combine(ee.Reducer.mean(), '', true),
    geometry: BBOX, scale: 500, maxPixels: 1e9
  })
...
// ── 5a. CALIBRATION MEASUREMENT — K67 tower pixel ────────────────────
// Prints the satellite HRC at the K67 tower pixel. NOT a pass-fail gate
// — this measures the MOD11A1 cold-bias propagation through Rn assembly.
// Expected: 5.62 ± 0.1 (within documented biome limitation).
var K67 = ee.Geometry.Point([-54.959, -2.857]);
print(
  'K67 calibration: HRC at K67 pixel ' +
  '(matched-methodology reference = 7.65, ±2.0 documented MOD11A1 ' +
  'cold-bias range; expected ~5.62):',
...
print('Sample point count (Tapajós — expect ~30,559):', samplePoints.size());

// ── 7. Map preview ───────────────────────────────────────────────────
Map.addLayer(hrc,
  { min: 0, max: 10, palette: ['8B2500','D4550A','F4A623','C8D84A','1D9E75'] },
  'HRC v2.1.1 (Tapajós, 500m)'
);
Map.addLayer(K67, { color: 'red' },
  'K67 tower (calibration target 7.65; documented MOD11A1 cold-bias range)');

// ── 8. Export ────────────────────────────────────────────────────────
Export.table.toDrive({
  collection:     samplePoints,
  description:    'hrc_v2_1_1_tapajos_tiles',
  folder:         'EarthHRC',
  fileNamePrefix: 'hrc_v2_1_1_tapajos_tiles',
  fileFormat:     'CSV',
  selectors: [
